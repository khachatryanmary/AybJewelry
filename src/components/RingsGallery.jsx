...
const RingGallery = () => {
    ...
    useEffect(() => {
        const getRings = async () => {
            try {
                setLoading(true);
                const res = await axios.get(`${API_URL}/api/products?category=ring`);
                setProduct(res.data);
                ...
                console.error("Error fetching rings:", error.message);
            } finally {
                setLoading(false);
            }
        };
    ...
    const handleAddToCart = async (product) => {
        ...
        try {
            ...
            toast.error(t('ringGallery.cartError', { defaultValue: "Failed to add to cart" }));
        }
    };
    ...
    const toggleWishlist = async (product) => {
        ...
        try {
            if (isWished[product._id]) {
                ...
                toast.info(t('ringGallery.removedFromWishlist', { defaultValue: `${product.name} removed from wishlist` }));
            } else {
                await axios.post(`${API_URL}/api/wishlist/${userId}`, {
                    productId: product._id,
                });
                setIsWished(prev => ({ ...prev, [product._id]: true }));
                toast.success(t('ringGallery.addedToWishlist', { defaultValue: `${product.name} added to wishlist` }));
            }
        } catch (err) {
                                ...
                                <div className="flex w-full justify-between">
                                    ...
                                    <div className="flex gap-2 mt-2">
                                        <span
                                            ...
                                            title={addedToCart[_id] ? t('ringGallery.addedToCart') : t('ringGallery.addToCart')}
                                        >
