...
const BroochGallery = () => {
    ...
    useEffect(() => {
        const getBrooches = async () => {
            try {
                setLoading(true);
                const res = await axios.get(`${API_URL}/api/products?category=brooch`);
                setProduct(res.data);
                ...
                console.error("Error fetching brooches:", error.message);
            } finally {
                setLoading(false);
            }
        };
    ...
    const handleAddToCart = async (product) => {
        ...
        try {
            ...
            toast.success(t('broochGallery.addedToCart', { defaultValue: `${product.name} added to cart!` }));
            setTimeout(() => {
                setAddedToCart(prev => ({ ...prev, [product._id]: false }));
            }, 3000);
        } catch (error) {
            console.error("Failed to add to cart:", error.message);
            toast.error(t('broochGallery.cartError', { defaultValue: "Failed to add to cart" }));
        }
    };
    ...
    const toggleWishlist = async (product) => {
        ...
        try {
            if (isWished[product._id]) {
                ...
                toast.info(t('broochGallery.removedFromWishlist', { defaultValue: `${product.name} removed from wishlist` }));
            } else {
                await axios.post(`${API_URL}/api/wishlist/${userId}`, {
                    productId: product._id,
                });
                setIsWished(prev => ({ ...prev, [product._id]: true }));
                toast.success(t('broochGallery.addedToWishlist', { defaultValue: `${product.name} added to wishlist` }));
            }
        } catch (err) {
            console.error("Wishlist toggle failed:", err.message);
            toast.error(t('broochGallery.wishlistError', { defaultValue: "Error updating wishlist" }));
        }
    };
        ...
        <div className="w-full">
            <div className="w-[90%] flex flex-col justify-center items-center mx-auto">
                <div className="my-[30px] flex flex-col justify-center items-center text-[#0e0e53] text-center font-[Against] italic font-light">
                    <h2 className="text-[30px]">{t('broochGallery.title')}</h2>
                </div>
                                ...
                                <div className="flex w-full justify-between">
                                    ...
                                    <div className="flex gap-2 mt-2">
                                        <span
                                            ...
                                            title={addedToCart[_id] ? t('broochGallery.addedToCart') : t('broochGallery.addToCart')}
                                        >
                                        ...
                                        <span
                                            ...
                                            title={t('broochGallery.addToWishlist')}
                                        >
                ...
                {!loading && visibleCount < filteredProduct.length && (
                    ...
                    >
                        {t('broochGallery.loadMore') || 'Load More'}
                    </button>
                )}
            </div>
                    ...
                    >
                        <div className="bg-white rounded-[10px] p-[20px] w-[500px] flex flex-col items-center justify-center gap-[20px]">
                            ...
                            <h2 className="text-[25px] text-[#0e0e53]">
                                {t(`broochGallery.loginPrompt.${loginPromptType}`)}
                            </h2>
                            <Link to={`/${lng}/login`}>
