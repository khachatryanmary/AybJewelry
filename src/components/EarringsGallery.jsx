...
const EarringGallery = () => {
    ...
    useEffect(() => {
        const getEarrings = async () => {
            try {
                setLoading(true);
                const res = await axios.get(`${API_URL}/api/products?category=earring`);
                setProduct(res.data);
                ...
                console.error("Error fetching earrings:", error.message);
            } finally {
                setLoading(false);
            }
        };
    ...
    const handleAddToCart = async (product) => {
        ...
        try {
            ...
            toast.error(t('earringGallery.cartError', { defaultValue: "Failed to add to cart" }));
        }
    };
    ...
    const toggleWishlist = async (product) => {
        ...
        try {
            if (isWished[product._id]) {
                ...
                toast.info(t('earringGallery.removedFromWishlist', { defaultValue: `${product.name} removed from wishlist` }));
            } else {
                await axios.post(`${API_URL}/api/wishlist/${userId}`, {
                    productId: product._id,
                });
                setIsWished(prev => ({ ...prev, [product._id]: true }));
                toast.success(t('earringGallery.addedToWishlist', { defaultValue: `${product.name} added to wishlist` }));
            }
        } catch (err) {
                                ...
                                <div className="flex w-full justify-between">
                                    ...
                                    <div className="flex gap-2 mt-2">
                                        <span
                                            ...
                                            title={addedToCart[_id] ? t('earringGallery.addedToCart') : t('earringGallery.addToCart')}
                                        >
